...
def send_telegram_msg(bot_token, chat_id, message_text):
    ...
    try:
        ...
        print(f"Response ({chat_id}):", r.status_code, r.text)
    except Exception as e:
        print(f"Error sending message: {e}")

def run_signals_engine():
    print("Testing connection...")
    send_telegram_msg(VIP_BOT_TOKEN, VIP_CHANNEL_ID, "🚀 *VIP Bot System Connected Successfully!*")
    send_telegram_msg(FREE_BOT_TOKEN, FREE_CHANNEL_ID, "🚀 *Free Bot System Connected Successfully!*")

    url = "https://api.binance.com/api/v3/ticker/24hr"
    try:
        res = requests.get(url).json()
        usdt_pairs = [x for x in res if isinstance(x, dict) and x.get("symbol", "").endswith("USDT") and "UP" not in x.get("symbol", "") and "DOWN" not in x.get("symbol", "")]
        sorted_pairs = sorted(usdt_pairs, key=lambda x: float(x.get("quoteVolume", 0)), reverse=True)[:10]
    except Exception as e:
        print("Binance API error:", e)
        return

    for index, coin in enumerate(sorted_pairs):
        symbol = coin["symbol"]
