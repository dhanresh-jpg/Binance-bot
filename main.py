...
def dispatch_vip_signal(s):
    ...
    # Percentage calculation for targets and stop loss based on entry price
    p = s['price']
    tp1_pct = round(((s['tp1'] - p) / p) * 100, 2)
    tp2_pct = round(((s['tp2'] - p) / p) * 100, 2)
    tp3_pct = round(((s['tp3'] - p) / p) * 100, 2)
    sl_pct = round(((s['sl'] - p) / p) * 100, 2)
    
    tp1_str = f"+{tp1_pct}%" if tp1_pct >= 0 else f"{tp1_pct}%"
    tp2_str = f"+{tp2_pct}%" if tp2_pct >= 0 else f"{tp2_pct}%"
    tp3_str = f"+{tp3_pct}%" if tp3_pct >= 0 else f"{tp3_pct}%"
    sl_str = f"+{sl_pct}%" if sl_pct >= 0 else f"{sl_pct}%"
...
        f"🎯 <b>Target 1</b>: ${format_price(s['tp1'])} ({tp1_str})\n"

        f"🎯 <b>Target 2</b>: ${format_price(s['tp2'])} ({tp2_str})\n"

        f"🚀 <b>Target 3 (Max)</b>: ${format_price(s['tp3'])} ({tp3_str})\n"

        f"⛔ <b>Stop Loss</b>: ${format_price(s['sl'])} ({sl_str})\n"
