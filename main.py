...
# ==================== FIXED LIVE MARKET DATA (EXACT EXCHANGE PRICE) ====================
def get_market_data():
    valid_coins = []
    try:
...
def scan_and_dispatch(force_mode=False):
    ...
    if chosen_type == "FUTURES LONG":
        ...
        leverage = "Spot (1x)"
        tp1, tp2, tp3 = p * 1.025, p * 1.050, p * 1.090
        sl = p * 0.950
    else:
        signal_mode = "SPOT BREAKOUT BUY"
        leverage = "Spot (1x)"
        tp1, tp2, tp3 = p * 1.020, p * 1.045, p * 1.080
        sl = p * 0.965
...
def vip_webhook():
    try:
        ...
        if data and "message" in data:
            ...
            if text.startswith("/start"):
                send_telegram_msg(VIP_VIP_BOT_TOKEN if 'VIP_VIP_BOT_TOKEN' in globals() else VIP_BOT_TOKEN, chat_id, "Welcome to Binance Top 10 VIP Bot! You will receive exclusive signals here.")
    except Exception as e:
