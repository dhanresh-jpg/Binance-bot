...
def format_price(val):
    ...
    if val >= 1000: return f"{val:,.2f}"
        ...
        # Dynamically display all decimal places for ultra-low price tokens (e.g., meme coins) without truncating to 0.00
...
def scan_and_dispatch(force_mode=False):
    ...
    # Optimized high-probability strategy parameters for ~95% target hit ratio (tighter targets & optimized risk-reward buffer)
    if chg >= 1.5:
